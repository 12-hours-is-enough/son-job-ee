...
public class CompanyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
		    ...
		    if (success) {
		        response.sendRedirect("companyPage.jsp?message=정보가 성공적으로 업데이트되었습니다.");
		    } else {
		        response.sendRedirect("companyPage.jsp?message=정보 업데이트에 실패했습니다.");
		    }
		} catch (SQLException e) {
		    e.printStackTrace();  // 콘솔에 에러 로그 출력
		    response.sendRedirect("companyPage.jsp?message=데이터베이스 오류 발생");
		}
